...
test('css(String|Hash)', function() {
    ...
    equal(child[0].style.height, old, 'Make sure height isnt changed on NaN.');

    // Test null
    child.css('height', null);
    equal(child[0].style.height, old, 'Make sure height isnt changed on null.');

    old = child[0].style.fontSize;

    // Test NaN
    child.css('font-size', parseFloat('zoo'));
    equal(child[0].style.fontSize, old, 'Make sure font-size isnt changed on NaN.');

    // Test null
    child.css('font-size', null);
    equal(child[0].style.fontSize, old, 'Make sure font-size isnt changed on null.');
...
test('css(String, Object)', function() {
    ...
    equal(div.css('display'), display, 'Make sure that the display wasnt changed.');
...
test('jQuery.css(elem, "height") doesnt clear radio buttons (bug #1095)', function () {
    expect(4);
...
test('cant get css for disconnected in IE<9, see #10254 and #8388', function() {
    expect(2);
    var span, div;

    span = jQuery('<span/>').css('background-image', 'url(data/1x1.jpg)');
    notEqual(span.css('background-image'), null, 'cant get background-image in IE<9, see #10254');

    div = jQuery('<div/>').css('top', 10);
    equal(div.css('top'), '10px', 'cant get top in IE<9, see #8388');
});

test('cant get background-position in IE<9, see #10796', function() {
    ...
    for(; i < l; i++) {
        ...
        ok(div.css('background-position'), 'cant get background-position in IE<9, see #10796');
    }
});
...
test('Keep the last style if the new one isnt recognized by the browser (#14836)', function() {
    expect(2);
...
asyncTest('Clearing a Cloned Elements Style Shouldnt Clear the Original Elements Style (#8908)', 24, function() {
    ...
    jQuery.each(styles, function(index, style) {
        ...
        if (source.style[ style.name ] === undefined) {
            ok(true, style.name +  ': style isnt supported and therefore not an issue');
            ok(true);
        ...
        window.setTimeout(function() {
            ...
            ok(jQuery.inArray($source.css(style.name) !== -1, style.value),
                'Clearing clone.css() doesnt affect source.css(): ' + style.name +
                '; result: ' + $source.css(style.name) +
                '; expected: ' + style.value.join(','));

            ok(jQuery.inArray($children.css(style.name) !== -1, style.value),
                'Clearing clonedChildren.css() doesnt affect children.css(): ' + style.name +
...
(function() {
    ...
    if (exist) {
        test('Dont append px to CSS \'order\' value (#14049)', 1, function() {
            var $elem = jQuery('<div/>');
