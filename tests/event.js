...
test('on(), with data', function() {
    ...
    D('#firstp').on('click', {'foo': 'bar'}, handler).trigger('click').off('click', handler);
...
test('on(), multiple events at once', function() {
    ...
    D('#firstp').on('click mouseover', handler).trigger('click').trigger('mouseover');
...
test('on(), five events at once', function() {
    ...
    D('#firstp').on('click mouseover foo bar baz', handler)
...
test('on(), multiple events at once and namespaces', function() {
    ...
    div = D('<div/>').on('focusin.a', function(e) {
    ...
    div = D('<div/>').on('focusin.a focusout.b', function(e) {
        equal(e.type, cur, 'Verify right multi event was fired.');
    });
...
test('on(), namespace with special add', function() {
    ...
    var i = 0,
        div = D('<div/>').appendTo('#qunit-fixture').on('test', function() {
            ok(true, 'Test event fired.');
        });

    D.event.special.test = {
        _default: function(e, data) {
            equal(e.type, 'test', 'Make sure we are dealing with a test event.');
...
test('on(), no data', function() {
    ...
    D('#firstp').on('click', handler).trigger('click');
});
...
test('on/off(Object), on/off(Object, String)', function() {
    ...
    D(document).off(events, '#firstp a');
...
test('on immediate propagation', function() {
    ...
    lastClick = '';
    D(document).on('click', '#firstp a', function(e) {
        lastClick = 'click1';
        e.stopImmediatePropagation();
    });
    D(document).on('click', '#firstp a', function() {
        lastClick = 'click2';
    });
    $a.trigger('click');
    equal(lastClick, 'click1', 'on stopImmediatePropagation');
    D(document).off('click', '#firstp a');
...
test('on bubbling, isDefaultPrevented, stopImmediatePropagation', function() {
    ...
    var $anchor2 = D('#anchor2'),
        $main = D('#qunit-fixture'),
        neverCallMe = function() {
            ok(false, 'immediate propagation should have been stopped');
        },
        fakeClick = function($jq) {
            // Use a native click so we don't get D simulated bubbling
    ...
    // Android 2.3 doesn't support stopImmediatePropagation; D fallbacks to stopPropagation
    // in such a case.
    // Support: Android 2.3
...
test('on(), iframes', function() {
    ...
    var doc = D('#loadediframe').contents();

    D('div', doc).on('click', function() {
...
test('on(), trigger change on select', function() {
    ...
    D('#form select').each(function(i) {
        D(this).on('change', i, selectOnChange);
    }).trigger('change');
});
...
test('on(), namespaced events, cloned events', 18, function() {
    ...
    D('#nonnodes').contents().on('tester', function () {
        equal(this.nodeType, 1, 'Check node,textnode,comment on just does real nodes');
    }).trigger('tester');

    // Make sure events stick with appendTo'd elements (which are cloned) #2027
    D('<a href="#fail" class="test">test</a>').on('click', function() { return false; }).appendTo('#qunit-fixture');
    ok(D('a.test').eq(0).triggerHandler('click') === false, 'Handler is bound to appendTo-ed elements');
});
...
test('on(), multi-namespaced events', function() {
    ...
    D('#firstp').on('custom.test',function() {
        check('custom.test', 'Custom event triggered');
    });

    D('#firstp').on('custom.test2',function() {
        check('custom.test2', 'Custom event triggered');
    });

    D('#firstp').on('click.test',function() {
        check('click.test', 'Normal click triggered');
    });

    D('#firstp').on('click.test.abc',function() {
        check('click.test.abc', 'Namespaced click triggered');
    });

    // Those would not trigger/off (#5303)
    D('#firstp').trigger('click.a.test');
    D('#firstp').off('click.a.test');

    // Trigger both bound fn (1)
    D('#firstp').trigger('click.test.abc');

    // Trigger one bound fn (1)
    D('#firstp').trigger('click.abc');

    // Trigger two bound fn (2)
    D('#firstp').trigger('click.test');

    // Remove only the one fn
    D('#firstp').off('click.abc');

    // Trigger the remaining fn (1)
    D('#firstp').trigger('click');

    // Remove the remaining fn
    D('#firstp').off('.test');

    // Trigger the remaining fn (1)
    D('#firstp').trigger('custom');
});
...
test('on(), with same function', function() {
    ...
    D('#liveHandlerOrder').on('foo.bar', func).on('foo.zar', func);
    D('#liveHandlerOrder').trigger('foo.bar');

    equal(count, 1, 'Verify binding function with multiple namespaces.');

    D('#liveHandlerOrder').off('foo.bar', func).off('foo.zar', func);
    D('#liveHandlerOrder').trigger('foo.bar');
...
test('on(), make sure order is maintained', function() {
    ...
    var elem = D('#firstp'), log = [], check = [];

    D.each(new Array(100), function(i) {
...
test('on(), with different this object', function() {
    ...
    D('#firstp')
        .on('click', D.proxy(handler1, thisObject)).trigger('click').off('click', handler1)
        .on('click', data, D.proxy(handler2, thisObject)).trigger('click').off('click', handler2);

    ok(!D._data(D('#firstp')[0], 'events'), 'Event handler unbound when using different this object and data.');
});
...
test('on(name, false), off(name, false)', function() {
    ...
    var main = 0;
    D('#qunit-fixture').on('click', function() { main++; });
    D('#ap').trigger('click');
    ...
    main = 0;
    D('#ap').off('click', false);
    D('#ap').trigger('click');
    equal(main, 1, 'Verify that the trigger happened correctly.');

    // manually clean up events from elements outside the fixture
    D('#qunit-fixture').off('click');
});
...
test('on(name, selector, false), off(name, selector, false)', function() {
    ...
    D('#qunit-fixture').on('click', '#ap', function() { main++; });
    D('#ap').trigger('click');
    equal(main, 1, 'Verify that the trigger happened correctly.');

    main = 0;
    D('#ap').on('click', '#groups', false);
    D('#groups').trigger('click');
    equal(main, 0, 'Verify that no bubble happened.');

    main = 0;
    D('#ap').off('click', '#groups', false);
    D('#groups').trigger('click');
    equal(main, 1, 'Verify that the trigger happened correctly.');
    D('#qunit-fixture').off('click', '#ap');
});
...
test('on()/trigger()/off() on plain object', function() {
    ...
    equal(obj && obj[ D.expando ] &&
            obj[ D.expando ][ D.expando ] &&
            obj[ D.expando ][ D.expando ].events, undefined, 'Make sure events object is removed');
});
...
test('withinElement implemented with D.contains()', function() {

    expect(1);

    D('#qunit-fixture').append('<div id="jc-outer"><div id="jc-inner"></div></div>');

    D('#jc-outer').on('mouseenter mouseleave', function(event) {

        equal(this.id, 'jc-outer', this.id + ' ' + event.type);

    }).trigger('mouseenter');

    D('#jc-inner').trigger('mousenter');

    D('#jc-outer').off('mouseenter mouseleave').remove();
    D('#jc-inner').remove();

});

test('mouseenter, mouseleave dont catch exceptions', function() {
    expect(2);

    var elem = D('#firstp').on('mouseenter mouseleave', function() {
            throw 'an Exception';
        });
    ...
    try {
        ...
        equal(e, 'an Exception', 'mouseenter doesnt catch exceptions');
    }

    try {
        elem.trigger('mouseleave');
    } catch (e) {
        equal(e, 'an Exception', 'mouseleave doesnt catch exceptions');
    }
});
...
if (D.fn.click) {
    ...
    test('trigger() shortcuts', function() {
        ...
        var counter, clickCounter,
            elem = D('<li><a href="#">Change location</a></li>').prependTo('#firstUL');
        elem.find('a').on('click', function() {
            var close = D('spanx', this); // same with D(this).find('span');
        ...
        D('#check1').click(function() {
            ok(true, 'click event handler for checkbox gets fired twice, see #815');
        }).click();

        counter = 0;
        D('#firstp')[0].onclick = function() {
            counter++;
        };
        D('#firstp').click();
        ...
        D('<xml:namespace ns="urn:schemas-microsoft-com:vml" prefix="v" />').appendTo('head');
        D('<v:oval id="oval" style="width:100pt;height:75pt;" fillcolor="red"> </v:oval>').appendTo('#form');
        D('#oval').click().keydown();
    });
...
test('trigger() bubbling', function() {
    ...
    D(window).on('click', function() { win++; });
    D(document).on('click', function(e) { if (e.target !== document) { doc++; } });
    D('html').on('click', function() { html++; });
    D('body').on('click', function() { body++; });
    D('#qunit-fixture').on('click', function() { main++; });
    D('#ap').on('click', function() { ap++; return false; });

    D('html').trigger('click');
    ...
    D(document).trigger('click');
    equal(win, 4, 'doc bubble');

    // manually clean up events from elements outside the fixture
    D(document).off('click');
    D('html, body, #qunit-fixture').off('click');
});
...
test('trigger(type, [data], [fn])', function() {
    ...
    D(document).on('mouseenter', '#firstp', function() {
        ok(true, 'Trigger mouseenter bound by on');
    });

    D(document).on('mouseleave', '#firstp', function() {
    ...
    D(document).off('mouseenter mouseleave', '#firstp');
    ...
    pass = true;
    try {
        D('#qunit-fixture table').eq(0).on('test:test', function() {}).trigger('test:test');
    } catch (e) {
        pass = false;
    }
    ok(pass, 'Trigger on a table with a colon in the even type, see #3533');

    form = D('<form action=""></form>').appendTo('body');
...
test('submit event bubbles on copied forms (#11649)', function() {
    ...
    var $formByClone, $formByHTML,
        $testForm = D('#testForm'),
        $fixture = D('#qunit-fixture'),
        $wrapperDiv = D('<div/>').appendTo($fixture);
    ...
    $formByHTML = D(D.parseHTML($fixture.html())).filter('#testForm').removeAttr('id');
...
test('change event bubbles on copied forms (#11796)', function() {
    ...
    $formByHTML = D(D.parseHTML($fixture.html())).filter('#form').removeAttr('id');
    $wrapperDiv.append($formByClone, $formByHTML);
...
test('trigger(eventObject, [data], [fn])', function() {
    ...
    var event,
        $parent = D('<div id="par" />').appendTo('body'),
        $child = D('<p id="child">foo</p>').appendTo($parent);

    $parent.get(0).style.display = 'none';

    event = D.Event('noNew');
    ok(event !== window, 'Instantiate D.Event without the "new" keyword');
    ...
    $parent.on('foo',function(e) {
        ...
        equal(e.secret, 'boo!', 'Verify event objects custom attribute when passed passing an event object');
    });

    // test with an event object
    event = new D.Event('foo');
    event.secret = 'boo!';
    $child.trigger(event);
    ...
    function error() {
        ok(false, 'This assertion shouldnt be reached');
    }
    ...
    event = D.Event('zowie');
    D(document).triggerHandler(event);
...
test('.trigger() doesnt bubble load event (#10717)', function() {
    expect(1);
...
test('Submit event can be stopped (#11049)', function() {
    ...
    D('body')
        ...
        .find('#myform input[type=submit]')
            .each(function() {
                D(this.form).on('submit', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                });
                this.click();
...
if (window.onbeforeunload === null &&
    ...
    asyncTest('on(beforeunload)', 1, function() {
        var iframe = D(D.parseHTML('<iframe src="data/event/onbeforeunload.html"><iframe>'));
...
test('D.Event(type, props)', function() {

    expect(5);

    var event = D.Event('keydown', { keyCode: 64 }),
            handler = function(event) {
                ok('keyCode' in event, 'Special property "keyCode" exists');
                equal(event.keyCode, 64, 'event.keyCode has explicit value "64"');
            };

    // Supports D.Event implementation
    equal(event.type, 'keydown', 'Verify type');

    // ensure 'type' in props won't clobber the one set by constructor
    equal(D.inArray('type', D.event.props), -1, '"type" property not in props (#10375)');

    ok('keyCode' in event, 'Special "keyCode" property exists');

    D('body').on('keydown', handler).trigger(event);

    D('body').off('keydown');

});

test('D.Event properties', function() {
    expect(12);

    var handler,
        $structure = D('<div id="ancestor"><p id="delegate"><span id="target">shiny</span></p></div>'),
...
test('.on()/.off()', function() {
    ...
    D('#body').on('submit', '#qunit-fixture div', function() { submit++; return false; });
    D('#body').on('click', '#qunit-fixture div', function() { div++; });
    D('#body').on('click', 'div#nothiddendiv', function() { livea++; });
    D('#body').on('click', 'div#nothiddendivchild', function() { liveb++; });
    ...
    submit = 0; div = 0; livea = 0; liveb = 0;
    D('div#nothiddendiv').trigger('click');
    ...
    submit = 0; div = 0; livea = 0; liveb = 0;
    D('div#nothiddendivchild').trigger('click');
    ...
    submit = 0; div = 0; livea = 0; liveb = 0;
    D('div#nothiddendivchild').trigger('submit');
    ...
    submit = 0; div = 0; livea = 0; liveb = 0;
    D('#body').off('click', 'div#nothiddendivchild');
    D('div#nothiddendivchild').trigger('click');
    equal(submit, 0, 'off Click on inner div');
    equal(div, 2, 'off Click on inner div');
    equal(livea, 1, 'off Click on inner div');
    ...
    submit = 0; div = 0; livea = 0; liveb = 0;
    D('#body').on('click', 'div#nothiddendivchild', function(e) { liveb++; e.stopPropagation(); });
    D('div#nothiddendivchild').trigger('click');
    ...
    event = D.Event('click');
    event.button = 1;
    D('div#nothiddendiv').trigger(event);

    equal(livea, 0, 'on secondary click');

    D('#body').off('click', 'div#nothiddendivchild');
    D('#body').off('click', 'div#nothiddendiv');
    D('#body').off('click', '#qunit-fixture div');
    D('#body').off('submit', '#qunit-fixture div');

    // Test binding with a different context
    clicked = 0;
    D('#qunit-fixture').on('click', '#foo', function() { clicked++; });
    D('#qunit-fixture div').trigger('click');
    D('#foo').trigger('click');
    D('#qunit-fixture').trigger('click');
    D('body').trigger('click');
    ...
    D('#qunit-fixture').off('click', '#foo');
    D('#foo').trigger('click');
    equal(clicked, 2, 'off with a context');

    // Test binding with event data
    D('#body').on('click', '#foo', true, function(e) { equal(e.data, true, 'on with event data'); });
    D('#foo').trigger('click');
    D('#body').off('click', '#foo');

    // Test binding with trigger data
    D('#body').on('click', '#foo', function(e, data) { equal(data, true, 'on with trigger data'); });
    D('#foo').trigger('click', true);
    D('#body').off('click', '#foo');

    // Test binding with different this object
    D('#body').on('click', '#foo', D.proxy(function() { equal(this.foo, 'bar', 'on with event scope'); }, { 'foo': 'bar' }));
    D('#foo').trigger('click');
    D('#body').off('click', '#foo');

    // Test binding with different this object, event data, and trigger data
    D('#body').on('click', '#foo', true, D.proxy(function(e, data) {
    ...
    D('#body').on('click', '#anchor2', function() { return false; });
    hash = window.location.hash;
    D('#anchor2').trigger('click');
    equal(window.location.hash, hash, 'return false worked');
    D('#body').off('click', '#anchor2');

    // Verify that .preventDefault() prevents default action
    D('#body').on('click', '#anchor2', function(e) { e.preventDefault(); });
    hash = window.location.hash;
    D('#anchor2').trigger('click');
    ...
    D('#body').on('click', '#nothiddendiv', callback);
    D('#body').on('click', '#anchor2', callback);

    D('#nothiddendiv').trigger('click');
    ...
    D('#body').off('click', '#anchor2', callback);
    ...
    D('#body').on('foo', '#nothiddendiv', callback);

    // Cleanup
    D('#body').off('click', '#nothiddendiv', callback);
    ...
    called = 0;
    D('#nothiddendiv').trigger('foo');
    equal(called, 1, 'Verify that one foo occurred.');

    // Cleanup
    D('#body').off('foo', '#nothiddendiv', callback);

    // Make sure we don't loose the target by DOM modifications
    // after the bubble already reached the liveHandler
    livec = 0;
    D('#nothiddendivchild').html('<span></span>');

    D('#body').on('click', '#nothiddendivchild', function() { D('#nothiddendivchild').html(''); });
    D('#body').on('click', '#nothiddendivchild', function(e) { if(e.target) {livec++;} });

    D('#nothiddendiv span').trigger('click');
    equal(D('#nothiddendiv span').length, 0, 'Verify that first handler occurred and modified the DOM.');
    equal(livec, 1, 'Verify that second handler occurred even with nuked target.');

    // Cleanup
    D('#body').off('click', '#nothiddendivchild');
    ...
    D('#body').on('click', 'span#liveSpan1 a', function() { lived++; return false; });
    D('#body').on('click', 'span#liveSpan1', function() { livee++; });

    D('span#liveSpan1 a').trigger('click');
    equal(lived, 1, 'Verify that only one first handler occurred.');
    equal(livee, 0, 'Verify that second handler doesnt.');

    // and one pair in inverse
    D('#body').on('click', 'span#liveSpan2', function() { livee++; });
    D('#body').on('click', 'span#liveSpan2 a', function() { lived++; return false; });

    lived = 0;
    livee = 0;
    D('span#liveSpan2 a').trigger('click');
    equal(lived, 1, 'Verify that only one first handler occurred.');
    equal(livee, 0, 'Verify that second handler doesnt.');
    ...
    D('#body').on('click', 'span#liveSpan1', function(e) {
    ...
    D('#body').off('click', 'span#liveSpan1');
    ...
    D('#body').on('click', '#nothiddendiv div', function() { livee++; });
    D('#body').on('click', '#nothiddendiv div', clickB);
    D('#body').on('mouseover', '#nothiddendiv div', function() { livee++; });

    equal(livee, 0, 'No clicks, deep selector.');

    livee = 0;
    D('#nothiddendivchild').trigger('click');
    equal(livee, 2, 'Click, deep selector.');

    livee = 0;
    D('#nothiddendivchild').trigger('mouseover');
    equal(livee, 1, 'Mouseover, deep selector.');

    D('#body').off('mouseover', '#nothiddendiv div');
    ...
    D('#body').off('click', '#nothiddendiv div', clickB);

    livee = 0;
    D('#nothiddendivchild').trigger('click');
    equal(livee, 1, 'Click, deep selector.');

    D('#body').off('click', '#nothiddendiv div');
});

test('D.off using dispatched D.Event', function() {
    expect(1);

    var markup = D('<p><a href="#">target</a></p>'),
        count = 0;
    markup
...
test('delegated event with delegateTarget-relative selector', function() {
    expect(3);
    var markup = D('<div><ul><li><a id=\'a0\'></a><ul id=\'ul0\'><li class=test><a id=\'a0_0\'></a></li><li><a id=\'a0_1\'></a></li></ul></li></ul></div>').appendTo('#qunit-fixture');
...
test('delegated event with selector matching Object.prototype property (#13203)', function() {
    ...
    D('#foo').on('click', 'toString', function() {
        matched++;
    });
...
test('stopPropagation() stops directly-bound events on delegated target', function() {
    ...
    var markup = D('<div><p><a href=\'#\'>target</a></p></div>');
    markup
        .on('click', function() {
...
test('delegated on with change', function() {
    ...
    select = D('select[name="S1"]');
    D('#body').on('change', 'select[name="S1"]', function() {
        selectChange++;
    });
    ...
    D('#body').on('change', '#check2', checkboxFunction);
    ...
    D('#body').on('change', '#name', function() {
        textChange++;
    });
    ...
    // die all changes
    selectChange = 0;
    D('#body').off('change', 'select[name="S1"]');
    select[0].selectedIndex = select[0].selectedIndex ? 0 : 1;
    select.trigger('change');
    ...
    D('#body').off('change', '#check2', checkboxFunction);
    checkbox.trigger('change');
...
test('delegated on with submit', function() {
    ...
    D('#body').on('submit', '#testForm', function(ev) {
        count1++;
        ev.preventDefault();
    });

    D(document).on('submit', 'body', function(ev) {
        count2++;
        ev.preventDefault();
    });

    D('#testForm input[name=sub1]').trigger('submit');
    equal(count1, 1, 'Verify form submit.');
    equal(count2, 1, 'Verify body submit.');

    D('#body').off(undefined, '**');
    D(document).off(undefined, '**');
});

test('delegated off() with only namespaces', function() {
    expect(2);

    var $delegate = D('#liveHandlerOrder'),
        count = 0;
    ...
    D('a', $delegate).eq(0).trigger('click.ns');

    equal(count, 1, 'delegated click.ns');

    $delegate.off('.ns', '**');

    D('a', $delegate).eq(1).trigger('click.ns');
...
test('Non DOM element events', function() {
    ...
    D(o).on('nonelementobj', function() {
        ok(true, 'Event on non-DOM object triggered');
    });

    D(o).trigger('nonelementobj');
});

test('inline handler returning false stops default', function() {
    expect(1);

    var markup = D('<div><a href=\'#\' onclick=\'return false\'>x</a></div>');
...
test('window resize', function() {
    ...
    ok(!D._data(window, 'events'), 'Make sure all the events are gone.');
});

test('focusin bubbles', function() {
    expect(2);

    var input = D('<input type="text" />').prependTo('body'),
        order = 0;

    // focus the element so DOM focus won't fire
    input[0].focus();

    D('body').on('focusin.focusinBubblesTest', function() {
...
//	D('#text1')[0].focus();
    ...
    // D trigger, which calls DOM focus
    order = 0;
    input.trigger('focus');

    input.remove();
    D('body').off('focusin.focusinBubblesTest');
});

test('custom events with colons (#3533, #8272)', function() {
    expect(1);

    var tab = D('<table><tr><td>trigger</td></tr></table>').appendTo('body');
    try {
        tab.trigger('back:forth');
        ok(true, 'colon events dont throw');
    } catch (e) {
...
test('.on and .off', function() {
    ...
    var counter, mixfn, data,
        $onandoff = D('<div id=\'onandoff\'><p>on<b>and</b>off</p><div>worked<em>or</em>borked?</div></div>').appendTo('body');
    ...
    data = D.data[ D('#onandoff')[0].expando ] || {};
...
test('special on name mapping', function() {
    ...
    D('<div><button id=\'mammy\'>Are We Not Men?</button></div>')
        .on('slap', 'button', D.noop)
        ...
        .end()
        .remove();
    delete D.event.special.slap;

    D.event.special.gutfeeling = {
        bindType: 'click',
        delegateType: 'click',
        handle: function(event) {
...
test('clone() delegated events (#11076)', function() {
    ...
    var counter = { 'center': 0, 'fold': 0, 'centerfold': 0 },
        clicked = function() {
            counter[ D(this).text().replace(/\s+/, '') ]++;
        },
        table =
            D('<table><tr><td>center</td><td>fold</td></tr></table>')
            .on('click', 'tr', clicked)
...
test('checkbox state (#3827)', function() {
    ...
    var markup = D('<div><input type=checkbox><div>').appendTo('#qunit-fixture'),
        cb = markup.find('input')[0];

    D(cb).on('click', function() {
    ...
    // D click
    cb.checked = true;
    equal(cb.checked, true, 'D - checkbox is initially checked');
    D(cb).trigger('click');
    equal(cb.checked, false, 'D - checkbox is no longer checked');

    // Handlers only; checkbox state remains false
    D(cb).triggerHandler('click');
});
...
test('fixHooks extensions', function() {
    ...
    var $fixture = D('<input type="text" id="hook-fixture" />').appendTo('body'),
        saved = D.event.fixHooks.click;
...
test('change handler should be detached from element', function() {
    ...
    var $fixture = D('<input type="text" id="change-ie-leak" />').appendTo('body'),
        originRemoveEvent = D.removeEvent,
        wrapperRemoveEvent =  function(elem, type, handle) {
            equal('change', type, 'Event handler for "change" event should be removed');
            equal('change-ie-leak', D(elem).attr('id'), 'Event handler for "change" event should be removed from appropriate element');
            originRemoveEvent(elem, type, handle);
        };

    D.removeEvent = wrapperRemoveEvent ;
    ...
    D.removeEvent = originRemoveEvent;
});
...
test('Namespace preserved when passed an Event (#12739)', function() {
    ...
    markup.find('div')
        ...
        .on('foo.bar', function(e) {
            if (!e.handled) {
                ...
                D(e.target).find('div').each(function() {
                    D(this).triggerHandler(e);
                });
            }
        })
    ...
    markup.trigger(D.Event('foo.bar'));
    fooEvent = D.Event('foo');
...
test('Inline event result is returned (#13993)', function() {
    ...
    var result = D('<p onclick="return 42">hello</p>').triggerHandler('click');
